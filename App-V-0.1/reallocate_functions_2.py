...
def category_totalizer(current_accounts):
    ...
    for category in category_names:
        ...
        for account in current_accounts:
            #troubleshooting-#
            total_category_values[category_number] += int(account[1][category_number])
            #print(account[1][category_number])
            #print(category)
            #print(account[1])
            #print(str(category_number))
        category_number += 1
...
def accounts_rule_scan(current_account_values):
    '''
    ...
    for account in current_account_values:
        ...
        #print(type(str(account[0][6])))
        #print(type('N'))

        if str(account[0][6]) == 'N':
            account[0].append('fixed')
            

        elif (str(account[0][4])) == 'NQ' and (str(account[0][6])) == 'Y':
            account[0].append('taxed')        
            

        elif (str(account[0][4])) == 'Q' and (str(account[0][5])) == 'Def' and (str(account[0][6])) == 'Y':
            account[0].append('tax_deferred') 


        elif (str(account[0][4])) == 'Q' and (str(account[0][5])) == 'Exempt' and (str(account[0][6])) == 'Y':
            account[0].append('tax_exempt') 

        else: 
        	account[0].append('unkown')
        	print('Error: Unkown account rules found')

        accounts.append(account[0][7])
    #print(accounts)
...
def fixed_taxed_vs_desired_allocation(desired, current, rules):
    ...
    for rule in rules:
        ...
        if str(rule) == 'fixed' or str(rule) == 'taxed':
            count = 0
    ...
    print('Fixed and taxed accounts within desired allocation range.')
    return 


#def find_rules(current_accounts)
...
def reallocate(needed_change, current_account_values, account_rules, group_rule, max_taxed_sales = 0):
    ...
    #find accounts with rule   <---Change this to find new set of rules, 3-4 spots in stead of 1
